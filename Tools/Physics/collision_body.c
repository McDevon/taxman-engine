...
void coll_set_body_rect_to_parent(CollisionBody *self)
{
    ...
    if (self->body_rect.size.width == nb_zero || self->body_rect.size.height == nb_zero) {
        self->body_rect = rect_make(-nb_mul(parent->anchor.x, parent->size.width), -nb_mul(parent->anchor.y, parent->size.height), parent->size.width, parent->size.height);
    }
}
...
CollisionBody *coll_create()
{
    ...
    coll->body_rect = rect_make(nb_zero, nb_zero, nb_zero, nb_zero);
